...
AEM Sites Optimizer會根據您的設定自動套用替代文字更新：

* **Edge Delivery Services** — 更新來原始檔（例如，Google Docs或SharePoint）。
* **AEM as a Cloud Service** — 透過具有版本設定和遞補支援的內容API直接寫入更新。
* **數位資產管理（選用）** — 更新資產層級的替代文字（如果適用）。
