...
AEM Preflight對話方塊頂端是反映整體稽核結果的使用者進度列。 它會顯示通過但無問題的機會百分比，以及在所有機會中找到的問題總數。 使用者進度列可協助作者快速評估整體的頁面健康狀態。 此條標示有色彩 — 紅色代表完成不到三分之一的商機，橘色代表完成三分之一至三分之二的商機，綠色代表完成超過三分之二的商機。 當稽核仍在執行時，進度列會以藍色顯示。
